...
import { sportIdentService } from '../services/sportIdentService';
...
const EventDayDashboard: React.FC = () => {
  ...
  // Debug function to inspect localStorage directly
  const debugLocalStorage = () => {
    try {
      const rawData = localStorage.getItem('meos_local_entries');
      if (rawData) {
        const parsedData = JSON.parse(rawData);
        console.log('[EventDay] Raw localStorage data:', parsedData);
        
        if (parsedData.length > 0) {
          console.log('[EventDay] First entry raw structure:', parsedData[0]);
          const classInfo = parsedData.map((entry: any) => ({
            name: `${entry.name?.first} ${entry.name?.last}`,
            classId: entry.classId,
            className: entry.className
          }));
          console.log('[EventDay] All entries class info:', classInfo);
        }
      } else {
        console.log('[EventDay] No localStorage data found');
      }
    } catch (error) {
      console.error('[EventDay] Error reading localStorage:', error);
    }
  };
  
  // Debug function to add test entries with different classes
  const addTestEntries = () => {
    ...
    testClasses.forEach((cls, index) => {
      localEntryService.addEntry({
        name: {
          first: `Test${index + 1}`,
          last: `Runner${cls.name}`
        },
        club: 'Test Club',
        birthYear: '1990',
        sex: 'M',
        ...
        isHiredCard: index === 0, // Make first test entry a hired card
        fee: 25,
        importedFrom: 'manual'
      });
    });
    
    message.success(`Added 4 test entries with different classes`);
    loadEntries();
  };
  ...
  useEffect(() => {
    ...
    debugLocalStorage(); // Debug localStorage first
    loadEntries();
    loadMeosClasses();
    ...
    return () => {
      console.log('[EventDay] Component unmounting, clearing selections...');
      setSelectedRowKeys([]);
    };
  }, []);
  ...
  const handleCardRead = async (card: SICard) => {
    console.log('[EventDay] Card read:', card);
    
    // Set the scanned card data
    setScannedCard(card);
    ...
    const hiredCardStatus = await meosHiredCardService.checkCard(card.cardNumber);
    console.log('[EventDay] Hired card check result:', hiredCardStatus);
    setIsHiredCard(hiredCardStatus.isHiredCard);
    
    if (exactMatch) {
      console.log('[EventDay] Found exact match:', exactMatch);
      setMatchedEntry(exactMatch);
      setSuggestedEntries([]);
      
      // Show confirmation modal and automatically open edit mode
      ...
      const suggestedEntries = entries.filter(entry => 
        entry.status === 'pending' && entry.issues.needsRentalCard
      );
      
      console.log('[EventDay] Found suggested entries for rental:', suggestedEntries);
    ...
    // Display additional context in console  
    if (hiredCardStatus.isHiredCard) {
      console.log('[EventDay] This is a confirmed hired card from MeOS');
    } else if (exactMatch) {
      console.log('[EventDay] Card matches registered runner');
    } else if (suggestedEntries.length > 0) {
      console.log('[EventDay] Card could be assigned to runners needing cards');
    } else {
      console.log('[EventDay] Card not recognized and no rental assignments available');
    }
  };
  ...
  // Handle confirmation modal check-in (direct check-in without editing)
  const handleConfirmCheckIn = async (entry: LocalEntry) => {
    if (!scannedCard) return;
    
    // Update the entry with the scanned card number if it's a rental assignment
    const updatedCardNumber = entry.issues.needsRentalCard ? scannedCard.cardNumber.toString() : entry.cardNumber;
    ...
    // Update entry with scanned card number if needed
    if (entry.issues.needsRentalCard) {
      const updatedEntry = localEntryService.updateEntry(entry.id, {
        cardNumber: updatedCardNumber,
        isHiredCard: isHiredCard
      });
      if (updatedEntry) {
        // Check in with the updated entry
        await handleCheckInEntry(updatedEntry);
  ...
  const handleCardReaderEntryCheckedIn = (entry: LocalEntry, cardNumber: string) => {
    console.log('[EventDay] Entry checked in via card reader:', entry, cardNumber);
    loadEntries(); // Refresh the entries table
    message.success(`${entry.name.first} ${entry.name.last} checked in with card ${cardNumber}`);
  };
          ...
          <Col>
            <Space>
              ...
              <Button
                ...
                title="Add test entries with different classes for debugging"
              >
                Add Test Classes
              </Button>
              <Button
                type="dashed"
                onClick={async () => {
                  console.log('[EventDay] Testing MeOS sync functionality...');
                  try {
                    const { meosApi } = await import('../services/meosApi');
                    const isConnected = await meosApi.testConnection();
                    
                    if (isConnected) {
                      const meosEntries = await meosApi.getAllEntries();
                      console.log(`Found ${meosEntries.length} entries in MeOS:`, meosEntries);
                      message.success(`MeOS connection test: Found ${meosEntries.length} entries`);
                    } else {
                      message.warning('MeOS is not connected');
                    }
                  } catch (error) {
                    console.error('MeOS sync test failed:', error);
                    message.error('MeOS sync test failed');
                  }
                }}
                style={{ marginLeft: '8px' }}
                title="Test MeOS connection and entry sync"
              >
                Test MeOS Sync
              </Button>
              <Button
                ...
                onClick={() => {
                  Modal.confirm({
                    ...
                    content: (
                      <div>
                        <p>This will mark <strong>ALL selected entries</strong> as "Checked In" locally.</p>
                        <p style={{ color: '#fa8c16', marginTop: '12px' }}>
                          ⚠️ <strong>Use this if:</strong> You know these runners were checked in but your backup was from before check-in.
                        </p>
                        <p style={{ fontSize: '14px', color: '#666' }}>
                          This only updates local status - it doesn't submit to MeOS.
                        </p>
                      </div>
                    ),
                    icon: <CheckCircleOutlined style={{ color: '#52c41a' }} />,
                    okText: 'Mark as Checked In',
                    cancelText: 'Cancel',
                    onOk: () => {
                      const selectedEntries = entries.filter(entry => selectedRowKeys.includes(entry.id));
                      if (selectedEntries.length === 0) {
                      ...
                      selectedEntries.forEach(entry => {
                        if (entry.status === 'pending') {
                          localEntryService.updateEntry(entry.id, {
                            status: 'checked-in',
                            checkedInAt: new Date(),
                            submittedToMeosAt: new Date(), // Mark as if submitted to MeOS
                          });
                          updated++;
                        }
                      });
                      
                      message.success(`✅ Marked ${updated} entries as checked in`);
                ...
                style={{ marginLeft: '8px' }}
                title="Mark selected entries as checked in (for backup recovery)"
              ...
              <Col>
                <Space>
                  <Button
                    ...
                    onClick={() => {
                      if (matchedEntry) {
                        // Quick check-in without saving form changes
                        handleConfirmCheckIn(matchedEntry);
                      }
                    }}
                  >
                    Quick Check In (No Changes)
                  </Button>
                  <Button
                    type="primary"
                    size="large"
                  ...
                  <Row gutter={16} align="middle">
                    ...
                    <Col>
                      <Button
                        ...
                        icon={<IdcardOutlined />}
                        onClick={() => handleConfirmCheckIn(entry)}
                      >
